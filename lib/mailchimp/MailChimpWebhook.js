...
function MailChimpWebhook (options) {
	...
	this.listen = typeof options.listen !== 'undefined' ? options.listen : true;
	this.noServer = options.noServer || false;
...
(function(listener) {
	...
	if (!self.noServer) {
		...
		if (self.secure)
			server.setSecure(self.secure);

		if (self.listen)
			server.listen(self.httpPort);
